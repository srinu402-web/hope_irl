...
app.post('/api/payments/revolut/request', paymentLimiter, requireAuth(['client']), async (req, res) => {
    try {
        ...
        const { rows: [payment] } = await db.query(
            `INSERT INTO payments (client_id, plan_id, amount_eur, currency, method, gateway_ref, status, created_at) VALUES ($1,$2,$3,'EUR','revolut',$4,'pending',NOW()) RETURNING id`,
            [clients[0].id, plan_id, amount||0, tx_ref]);
    ...
    try {
        ...
        const { rows: [payment] } = await db.query(
            `SELECT p.*, cp.user_id AS client_user_id, sp.name AS plan_name, sp.applications_per_day
             FROM payments p JOIN client_profiles cp ON cp.id=p.client_id LEFT JOIN service_plans sp ON sp.id=p.plan_id WHERE p.id=$1`, [id]);
        if (!payment) return res.status(404).json({ error: 'Payment not found' });
        await db.query(`UPDATE payments SET status='paid', paid_at=NOW() WHERE id=$1`, [id]);
        await db.query(
            `INSERT INTO subscriptions (client_id, plan_id, status, starts_at, ends_at, payment_id) VALUES ($1,$2,'active',NOW(),NOW()+INTERVAL '30 days',$3)
             ON CONFLICT (client_id) DO UPDATE SET plan_id=$2,status='active',starts_at=NOW(),ends_at=NOW()+INTERVAL '30 days',payment_id=$3,updated_at=NOW()`,
            [payment.client_id, payment.plan_id, payment.id]);
...
app.get('/api/admin/payments', requireAuth(['admin']), async (req, res) => {
    const { rows } = await db.query(`
        SELECT p.*, u.full_name AS client_name, u.email AS client_email, sp.name AS plan_name
        FROM payments p
...
app.get('/api/me/payments', requireAuth(['client']), async (req, res) => {
    try {
        const { rows } = await db.query(`
            ...
            FROM payments p
            JOIN client_profiles cp ON cp.id=p.client_id
            LEFT JOIN service_plans sp ON sp.id=p.plan_id
...
app.get('/health', async (_req, res) => {
    try {
        ...
        return res.json({ status: 'ok', db: 'connected', env: process.env.NODE_ENV, timestamp: new Date().toISOString() });
    } catch {
...
app.use((err, _req, res, _next) => {
    ...
    if (err.message?.includes('Only PDF') || err.message?.includes('Images only')) {
        return res.status(400).json({ error: err.message });
    }
...
// ──────────────────────────────────────────────────────────────
// 27. START SERVER
// FIX: app.listen() is at the END, after all routes and SSE setup
// ──────────────────────────────────────────────────────────────
