...
// Fixes in v3:
//   - Render/Proxy safe: app.set('trust proxy', 1)
//   - Rate-limit split by route (login/register/forgot/resend OTP)
//   - SSE route correctly skipped from global limiter
//   - OPTIONS + health skipped from global limiter
// ============================================================
...
async function sendWhatsAppAlert(message) {
    try {
        ...
        }).on('error', (e) => console.warn('WA failed:', e.message));
    } catch (err) {
...
// ──────────────────────────────────────────────────────────────
// INPUT SANITIZATION + LIMITER HELPERS
// ──────────────────────────────────────────────────────────────
...
function emailLimiterKey(req) {
    const email = sanitizeEmail(req.body?.email) || 'unknown';
    return `${req.ip}:${email}`;
}
...
const resendOtpLimiter = rateLimit({
    ...
    standardHeaders: true,
    legacyHeaders: false,
    message: { error: 'Too many OTP resend attempts. Please wait.' },
    keyGenerator: emailLimiterKey,
});
...
app.delete('/api/admin/clients/:id', requireAuth(['admin']), async (req, res) => {
    try {
        ...
        asgn.forEach(a => pushSSE(a.employee_user_id, 'assignment_updated', {
...
app.post('/api/payments/revolut/request', paymentLimiter, requireAuth(['client']), async (req, res) => {
    try {
        ...
        if (!plan_id || !tx_ref) {
            return res.status(400).json({ error: 'plan_id and tx_ref required' });
        }
...
app.patch('/api/admin/payments/:id', requireAuth(['admin']), async (req, res) => {
    ...
    const { rows } = await db.query(
        `UPDATE payments
         SET status=$1, paid_at=CASE WHEN $1::text='paid' THEN NOW() ELSE paid_at END
         WHERE id=$2
         RETURNING *`,
        [status, req.params.id]
    );
...
app.get('/api/employee/clients', requireAuth(['employee', 'admin']), async (req, res) => {
    const { rows } = await db.query(`
        SELECT
            ...
            s.status AS subscription_status,
            sp.name AS plan_name,
            COALESCE(sp.applications_per_day, 15) AS applications_per_day,
            (SELECT COUNT(*) FROM job_applications ja WHERE ja.client_id = cp.id) AS total_applications,
