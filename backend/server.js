...
app.post('/api/applications', requireAuth(['employee', 'admin']), async (req, res) => {
    ...
    // Broadcast to admins/employees so their dashboards update in real-time
    try { broadcastSSE('application_changed', { client_id, action: 'created' }); } catch {}
...
app.patch('/api/applications/:id/status', requireAuth(['employee', 'admin']), async (req, res) => {
    ...
    // Broadcast for admin/employee dashboards
    try { broadcastSSE('application_changed', { client_id: rows[0].client_id, action: 'status' }); } catch {}

    return res.json(rows[0]);
});

app.delete('/api/applications/:id', requireAuth(['employee', 'admin']), async (req, res) => {
    const { rows } = await db.query('DELETE FROM job_applications WHERE id=$1 RETURNING id, client_id', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Application not found' });

    // Broadcast for admin/employee dashboards
    try { broadcastSSE('application_changed', { client_id: rows[0].client_id, action: 'deleted' }); } catch {}
