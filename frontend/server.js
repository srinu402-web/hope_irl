...
// Updates:
//   - Manual Revolut payment no longer requires tx_ref
//   - Backend auto-generates payment request reference
//   - WhatsApp admin alert sent for every manual payment request
//   - Backward compatible with old frontend sending tx_ref
//   - app.set('trust proxy', 1) added for Render
//   - SSE limiter skip path corrected to /api/realtime/events
// ============================================================
...
const app = express();
app.set('trust proxy', 1); // Important for Render / reverse proxy
...
// ──────────────────────────────────────────────────────────────
// 2. AWS SES EMAIL + WHATSAPP ALERTS
// ──────────────────────────────────────────────────────────────
...
async function sendWhatsAppAlert(message) {
    try {
        ...
        https
            ...
            .on('error', (e) => {
                console.warn('WA failed:', e.message);
            });
    } catch (err) {
        console.warn('WA error:', err.message);
    }
}

function generatePaymentRequestRef() {
    return `WA-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
}

async function sendPaymentConfirmedEmail(clientEmail, clientName, planName, amount, ref) {
    await sendEmail({
        to: clientEmail,
        subject: 'HOPE_IRL — Payment Confirmed & Subscription Activated!',
        html: `
        <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:32px;background:#f9fafb;border-radius:16px;">
            <div style="background:linear-gradient(135deg,#667eea,#764ba2);padding:28px;border-radius:12px;text-align:center;margin-bottom:28px;color:white;">
                <h1 style="margin:0;">HOPE_IRL</h1>
            </div>
            <h2>Payment Confirmed!</h2>
            <p>Hello <strong>${clientName}</strong>, your <strong>${planName}</strong> subscription is now <strong style="color:#22c55e;">ACTIVE</strong>.</p>
            ...
            <div style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:20px;margin:20px 0;">
                ...
                <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f3f4f6;">
                    ...
                    <strong style="color:#7e22ce;font-size:20px;">EUR${parseFloat(amount || 0).toFixed(2)}</strong>
                </div>
                <div style="display:flex;justify-content:space-between;padding:10px 0;">
                    <span style="color:#9ca3af;">Reference</span>
                    <span style="font-family:monospace;">${ref || 'N/A'}</span>
                </div>
            </div>

            <p style="color:#9ca3af;font-size:12px;text-align:center;">Thank you for choosing HOPE_IRL</p>
        </div>`,
        text: `Payment confirmed! ${planName} subscription active. Ref: ${ref || 'N/A'}`,
    });
}
...
async function sendExpiryReminderEmail(clientEmail, clientName, planName, expiryDate, daysLeft) {
    ...
    await sendEmail({
        ...
        <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:32px;">
            ...
            <div style="background:#fff;border:2px solid ${color};border-radius:12px;padding:20px;margin:20px 0;text-align:center;">
                <p style="color:${color};font-weight:700;font-size:18px;margin:0;">
                    Your <strong>${planName}</strong> expires in <strong>${daysLeft} day(s)</strong>
                </p>
                <p style="color:#6b7280;margin:8px 0 0;">
                    ${new Date(expiryDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
            </div>
            <p>Renew now to continue your job search journey!</p>
        </div>`,
...
// OTP store (Redis recommended in real production)
...
function sanitizeInt(val, min = 0, max = 999999) {
    ...
    return Number.isNaN(n) ? null : Math.min(Math.max(n, min), max);
}

function sanitizeAmount(val, min = 0, max = 100000) {
    ...
    return async (req, res, next) => {
        ...
        try {
            ...
            if (roles.length && !roles.includes(payload.role)) {
                console.warn(
                    `🚨 Forbidden access: user=${payload.sub} role=${payload.role} needed=${roles} path=${req.path} ip=${req.ip}`
                );
...
app.post('/api/auth/login', authLimiter, async (req, res) => {
    ...
    try {
        ...
        // Enable this if you want verified-only logins
        // if (!user.is_verified) {
...
app.get('/api/me', requireAuth(), async (req, res) => {
    const { rows } = await db.query(
        `SELECT id, email, full_name, role, phone, avatar_url, is_verified, last_login_at
...
if (avatarUpload) {
    app.post('/api/me/avatar', requireAuth(), avatarUpload.single('avatar'), async (req, res) => {
        ...
        try {
            ...
            await audit(req.user.sub, 'update_avatar', { table: 'users', recordId: req.user.sub });
...
app.get('/api/applications', requireAuth(), async (req, res) => {
    ...
    if (req.user.role === 'admin') {
        query = `
            SELECT ja.*, u.full_name AS client_name, u.id AS client_user_id, eu.full_name AS employee_name
        ...
        query = `
            SELECT ja.*, u.full_name AS client_name
            FROM job_applications ja
            JOIN client_profiles cp ON cp.id = ja.client_id
            JOIN users u ON u.id = cp.user_id
        ...
        query = `
            SELECT ja.*, eu.full_name AS employee_name
            FROM job_applications ja
...
app.post('/api/applications', requireAuth(['employee', 'admin']), async (req, res) => {
    ...
    const { rows } = await db.query(
        `INSERT INTO job_applications
         (client_id, employee_id, company_name, job_title, job_url, location, portal, notes, salary_range)
...
app.get('/api/admin/stats', requireAuth(['admin']), async (req, res) => {
    const [
        ...
        db.query(`
            SELECT ja.company_name, ja.job_title, ja.status, u.full_name AS client_name, ja.created_at
            FROM job_applications ja
            JOIN client_profiles cp ON cp.id=ja.client_id
            JOIN users u ON u.id=cp.user_id
        ...
        db.query(`
            SELECT u.full_name, u.email, sp.name AS plan_name, s.ends_at,
                   EXTRACT(DAY FROM s.ends_at-NOW())::int AS days_left
            FROM subscriptions s
            JOIN client_profiles cp ON cp.id=s.client_id
            JOIN users u ON u.id=cp.user_id
...
app.delete('/api/admin/employees/:id', requireAuth(['admin']), async (req, res) => {
    ...
    broadcastSSE('assignment_updated', {
...
app.post('/api/admin/assign', requireAuth(['admin']), async (req, res) => {
    ...
    if (
        epCheck.rows.length &&
        parseInt(epCheck.rows[0].assigned, 10) >= epCheck.rows[0].max_clients
    ) {
    ...
    await audit(req.user.sub, 'assign_client', {
...
app.delete('/api/admin/assign/:client_profile_id', requireAuth(['admin']), async (req, res) => {
    ...
    broadcastSSE('assignment_updated', {
        client_id: req.params.client_profile_id,
        unassigned: true,
    });
...
app.get('/api/admin/services', requireAuth(['admin']), async (req, res) => {
    const { rows } = await db.query(
        'SELECT * FROM service_plans ORDER BY is_active DESC, price_eur ASC'
    );
    return res.json(rows);
});
...
app.get('/api/services', async (req, res) => {
    const { rows } = await db.query(
        'SELECT * FROM service_plans WHERE is_active=TRUE ORDER BY price_eur ASC'
    );
    return res.json(rows);
});
...
// MANUAL REVOLUT PAYMENT REQUEST
// Old frontend can still send tx_ref.
// New frontend can omit tx_ref and backend will auto-generate request_ref.
app.post('/api/payments/revolut/request', paymentLimiter, requireAuth(['client']), async (req, res) => {
    try {
        const plan_id = sanitizeUUID(req.body.plan_id) || sanitizeStr(req.body.plan_id, 64);
        const providedTxRef = sanitizeStr(req.body.tx_ref, 100);
        const client_note = sanitizeStr(req.body.client_note || req.body.note || '', 500);
        const requestedPlanName = sanitizeStr(req.body.plan_name, 100);
        const requestedAmount = sanitizeAmount(req.body.amount, 1, 10000);
        ...
        const { rows: plans } = await db.query(
            'SELECT id, name, price_eur, is_active FROM service_plans WHERE id=$1',
            [plan_id]
        );
        ...
        const finalPlanName = requestedPlanName || plan.name;
        const finalAmount = requestedAmount || parseFloat(plan.price_eur || 0);
        const requestRef = providedTxRef || generatePaymentRequestRef();
        ...
        const { rows: paymentRows } = await db.query(
            `INSERT INTO payments
             (subscription_id, client_id, amount_eur, currency, method, gateway_ref, status, created_at)
             VALUES ($1,$2,$3,'EUR','revolut',$4,'pending',NOW())
             RETURNING id, gateway_ref`,
            [subId, clients[0].id, finalAmount, requestRef]
        );
        ...
        admins.forEach(a => {
            pushSSE(a.id, 'payment_pending', {
                payment_id: payment.id,
                request_ref: payment.gateway_ref,
        ...
        const waMessage = [
            ...
            `Plan: ${finalPlanName}`,
            `Amount: EUR${Number(finalAmount).toFixed(2)}`,
            `Request ID: ${payment.gateway_ref}`,
            client_note ? `Note: ${client_note}` : null,
        ]
            .filter(Boolean)
            .join('\n');
        ...
        return res.status(201).json({
            ...
            payment_id: payment.id,
            request_ref: payment.gateway_ref,
            payment_request_ref: payment.gateway_ref,
...
app.patch('/api/admin/payments/:id/confirm', requireAuth(['admin']), async (req, res) => {
    try {
        ...
        await audit(req.user.sub, 'confirm_payment', {
            table: 'payments',
            recordId: id,
        });
...
// ──────────────────────────────────────────────────────────────
// 22. CLIENT — SUBSCRIPTION / PAYMENTS
// ──────────────────────────────────────────────────────────────
...
app.post('/api/ats/analyze', uploadLimiter, requireAuth(), atsMiddleware, async (req, res) => {
    ...
    try {
        ...
        await db.query(
            `UPDATE documents
             SET ats_score=$1
             WHERE user_id=$2 AND doc_type='cv' AND is_primary=TRUE`,
...
// ──────────────────────────────────────────────────────────────
// 26. START SERVER + ERROR HANDLERS
// ──────────────────────────────────────────────────────────────
